...
const screenAudioCapture = new ApplicationAudioCaptureController(
  "cove:screen-audio:chunk",
);
...
app.on(
  ...
  (event, webContents, url, _error, _certificate, callback) => {
    if (
      ...
      serverCertificatePolicy.allows(url)
    ) {
...
function createWindow() {
  ...
  const notifyWindowState = () => {
    if (!win.isDestroyed()) win.webContents.send("cove:window-state", win.isMaximized());
  };
  ...
  if (isDev) {
    // 将渲染进程的 WebRTC 日志同步到启动终端，便于直接定位加入后被清理、
    // 信令超时或媒体轨道错误；正式版不输出这些调试信息。
    win.webContents.on("console-message", (_event, level, message, line, sourceId) => {
      const source = sourceId ? `${path.basename(sourceId)}:${line}` : String(line);
  ...
  session.defaultSession.setPermissionCheckHandler(
    (webContents, permission, _origin, details) => {
      if (permission === "fullscreen")
        return webContents === win.webContents && details.isMainFrame;
      return [
        "media",
        "audioCapture",
        "videoCapture",
...
app.whenReady().then(() => {
  ipcMain.handle("cove:window:minimize", (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win || win.isDestroyed()) return false;
    win.minimize();
    return true;
  });
  ipcMain.handle("cove:window:toggle-maximize", (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win || win.isDestroyed()) return false;
  ...
  ipcMain.handle("cove:window:is-maximized", (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    return Boolean(win && !win.isDestroyed() && win.isMaximized());
  });
  ipcMain.handle("cove:window:close", (event) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win || win.isDestroyed()) return false;
    win.close();
    return true;
  ...
  ipcMain.handle("cove:clipboard:write-text", (event, value: unknown) => {
    if (event.sender !== mainWindow?.webContents || typeof value !== "string")
      return false;
    try {
      clipboard.writeText(value);
      return true;
    } catch (error) {
      console.error("[clipboard] 文本写入失败", error);
      return false;
    }
  });
  ...
  ipcMain.handle(
    "cove:remote-control:input",
    (event, sessionId: unknown, input: unknown) => {
      if (
        event.sender !== mainWindow?.webContents ||
  ...
  ipcMain.handle(
    "cove:update:install",
    () => updaterController?.installNow() ?? false,
  );
    ...
    async (event, sourceId: unknown) => {
      ...
      try {
        ...
        return {
          ok: false,
          error:
            error instanceof Error ? error.message : "无法启动应用音频捕获。",
        };
      }
    },
  );
  ...
  ipcMain.handle("cove:screen-audio:start", (event) => {
    ...
    try {
      ...
      return {
        ok: false,
        error:
          error instanceof Error ? error.message : "无法启动系统音频捕获。",
      };
    }
  });
  ...
  ipcMain.handle("cove:system-audio:start", (event) => {
    if (event.sender !== mainWindow?.webContents)
      return { ok: false, error: "无效的系统音频请求。" };
    try {
      systemAudioCapture.startExcludingProcess(event.sender, process.pid);
      return { ok: true };
    } catch (error) {
      console.error("[system-audio] 启动失败", error);
      return {
        ok: false,
        error:
          error instanceof Error ? error.message : "无法启动系统音频捕获。",
  ...
  ipcMain.handle("cove:system-audio:stop", (event) => {
    if (event.sender !== mainWindow?.webContents) return false;

...
contextBridge.exposeInMainWorld("coveApplicationAudio", {
  ...
  onChunk: (listener: (chunk: Uint8Array) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, chunk: Uint8Array) =>
      listener(chunk);
...
contextBridge.exposeInMainWorld("coveSystemAudio", {
  ...
  onChunk: (listener: (chunk: Uint8Array) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, chunk: Uint8Array) =>
      listener(chunk);
    ipcRenderer.on("cove:system-audio:chunk", handler);
    return () => ipcRenderer.off("cove:system-audio:chunk", handler);
  },
});

...
export function UserProfileModal({ userId, username, avatarUrl, remark = '', onSaveRemark, onClose }: Props) {
  ...
  return (
    <div className="user-profile-v2-scrim" onMouseDown={onClose}>
      <section className="user-profile-v2" onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="other-profile-title">
        ...
        <div className="px-7 pb-7">
          ...
          <div className="profile-id-card mt-5 rounded-2xl border border-white/[0.08] bg-black/20 p-4">
            ...
            <div className="mt-2 flex items-center gap-2">
              <code className="profile-id-value min-w-0 flex-1 select-all break-all text-sm text-cyan-100/75">{userId}</code>
          ...
          <div className="profile-remark-field mt-2 flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.06] px-4 focus-within:border-cyan-300/50">
            <UserRound size={18} className="text-white/35" />
            <input id="profile-remark" value={draft} onChange={event => setDraft(event.target.value)} maxLength={64} className="profile-remark-input min-w-0 flex-1 bg-transparent py-3 text-white outline-none placeholder:text-white/20" placeholder="添加仅自己可见的备注" autoFocus />
          </div>
